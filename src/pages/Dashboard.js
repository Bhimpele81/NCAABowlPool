...
export default function Dashboard({ state, updateState }) {
  ...
  return (
    <div>
      ...
      {/* Mobile cards — stacked layout, no scroll */}
          ...
          return (
            <div key={game.id} style={{
              ...
              <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                ...
                <span style={{ fontWeight:700, fontSize:13 }}>{game.name}</span>
              </div>

              {/* Buttons — span both rows */}
              <div style={{ display:'flex', gap:5, gridRow:'1 / 3', alignSelf:'center' }}>
              ...
              <div style={{ fontSize:12, color:'var(--text-muted)' }}>
                ...
                {settled && <span style={{ marginLeft:8 }}>
                  <span style={{ color: billDelta >= 0 ? 'var(--green)' : 'var(--red)', fontWeight:600 }}>B:{formatMoney(billDelta)}</span>
                  <span style={{ margin:'0 4px', color:'var(--text-dim)' }}>·</span>
                  <span style={{ color: donDelta >= 0 ? 'var(--green)' : 'var(--red)', fontWeight:600 }}>D:{formatMoney(donDelta)}</span>
                </span>}
              </div>
            </div>
          );
